...
def main(args):
    ...
    with mlflow.start_run():
        ...
        # === CRITICAL: Light pre-clean so GE sees proper types ===
        # Strip whitespace in all object columns (Telco CSV often has " " cells)
        obj_cols = df.select_dtypes(include=["object"]).columns
        for c in obj_cols:
            df[c] = df[c].astype("str").str.strip()

        # Coerce key numeric columns; blanks -> NaN -> numeric
...
python .\scripts\run_pipeline.py --input .\data\raw\Telco-Customer-Churn.csv --target Churn --mlflow_uri .\mlruns
